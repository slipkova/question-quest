        ...
        if event.type == KEYDOWN:
            if (event.key == K_DOWN and self.orientation == "vertical") or (
                ...
                if K_DOWN not in self.pressed_before or K_RIGHT not in self.pressed_before:
                    if self.active[0] == len(self.buttons) - 1:
                        self.active[0] = 0
                    else:
                        self.active[0] += 1
            ...
            if (event.key == K_UP and self.orientation == "vertical") or (
                ...
                if K_UP not in self.pressed_before or K_LEFT not in self.pressed_before:
                    if self.active[0] == 0:
                        self.active[0] = len(self.buttons) - 1
                    else:
                        self.active[0] -= 1

            if event.key == K_SPACE or event.key == K_RETURN:
                if K_SPACE not in self.pressed_before or K_RETURN not in self.pressed_before:
                    if self.buttons[self.active[0]].pointer:
                        self.buttons[self.active[0]].click()
                    if self.buttons[self.active[0]].event == "back":
                        self.running = False
                    if self.buttons[self.active[0]].event == "quit":
                        self.running = False
                        GameObject.running = False
                        return False
                    if self.buttons[self.active[0]].event == "quit game":
                        pygame.quit()
                        sys.exit()
                    if self.buttons[self.active[0]].event in {"attack", "defense", "run"}:
                        self.buttons[self.active[0]].click()
                        return
                    self.active[0] = 0
